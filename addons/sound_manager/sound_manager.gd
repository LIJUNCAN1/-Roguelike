...
# Project adapter: keep the existing Music/SFX bus contract used by both
# settings panels instead of creating three competing sound-effect buses.
var sound_effects: SoundEffectsPlayer = SoundEffectsPlayer.new("SFX", 8)
var ui_sounds: SoundEffectsPlayer = SoundEffectsPlayer.new("SFX", 8)
...
func shutdown() -> void:
	# Project lifecycle adapter: release pooled streams explicitly so headless
	# smoke tests and scene teardown do not retain AudioStreamPlayback objects.
	for pool in [sound_effects, ui_sounds, ambient_sounds, music]:
		for tween: Tween in pool._tweens.values():
			if is_instance_valid(tween):
				tween.kill()
		pool._tweens.clear()
		for player: AudioStreamPlayer in (
			pool.available_players + pool.busy_players
		):
			player.stop()
			player.stream = null
		pool.busy_players.clear()
