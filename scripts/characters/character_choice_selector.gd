class_name CharacterChoiceSelector
extends Node2D

signal character_entered(choice_index: int)
...
func configure(
	character_choices: Array[CharacterData],
	player_node: Node2D
) -> void:
	choices.assign(character_choices)
	player = player_node
	is_active = false
	_arming_frames = 2
	for index in chambers.size():
...
func _on_chamber_body_entered(
	...
	if (
		...
		return
	deactivate()
	call_deferred("_emit_character", choice_index)


func _emit_character(choice_index: int) -> void:
	character_entered.emit(choice_index)
