class_name CompanionChoiceSelector
extends Node2D

signal companion_entered(choice_index: int)
...
var choices: Array[CompanionData] = []
var player: Node2D
...
func configure(
	companion_choices: Array[CompanionData],
	player_node: Node2D
) -> void:
	choices.assign(companion_choices)
	player = player_node
	is_active = false
	_arming_frames = 2
	for index in chambers.size():
...
func _on_chamber_body_entered(
	...
	call_deferred("_emit_companion", choice_index)


func _emit_companion(choice_index: int) -> void:
	companion_entered.emit(choice_index)
